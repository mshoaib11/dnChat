			...
			if(entrySenders.getKey().equals(chatNumber)){
				for (Entry<String, Socket> entry : messageRecievers.entrySet()){
					if(entry.getValue().equals(connection)){
						...
						String userNameofSender = connectionToNameMapping.get(messageSenders.get(chatNumber));
						//System.out.println("Recieved an ACK from "+currentUsers.get(userNumberOfAcknowledger)+" for a message sent by "+userNameofSender);
						try {
	...
	if(!messageSenders.containsKey(chatNumber)){
		...
		if(currentChatNumber < MaxChatNumber){
			...
			if(messageRecepients.equals("*")){
				...
				String SENDcommand = "SEND "+chatNumber+"\r\n"+ userNumberOfSender+"\r\n"+messageToSend;
				//System.out.println("User "+currentUsers.get(userNumberOfSender)+" sends a message to "+currentUsers.get(messageRecepients)+"! ");
